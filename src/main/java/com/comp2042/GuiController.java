    ...
    public class GuiController implements Initializable {
        ...
        public void initialize(URL location, ResourceBundle resources) {
            ...
            if (fontUrl != null) {
                ...
                LOGGER.warning("Font file 'digital.ttf' not found");
            }
//            initialize background music
            try {
                URL bgmUrl = getClass().getResource("/bgm.mp3");
                if (bgmUrl != null) {
                    Media bgm = new Media(bgmUrl.toString());
                    bgmPlayer = new MediaPlayer(bgm);
                    bgmPlayer.setCycleCount(MediaPlayer.INDEFINITE);
                    bgmPlayer.setVolume(1.0);
                    bgmPlayer.play();
                } else {
                    LOGGER.warning("Background music file 'bgm.mp3' not found");
                }
            } catch (Exception e) {
                LOGGER.log(Level.SEVERE, "Error loading background music", e);
            }
            initNextBrickPanel();
            initHoldBrickPanel();
            initPauseOverlay();
    ...
    //hard drop with space
        ...
        private void initPauseOverlay() {
            if (pauseOverlay == null) {
                LOGGER.severe("pauseOverlay not injected");
                return;
            }
            ...
            try {
                ...
                if (is != null) {
                    ...
                    LOGGER.warning("pause.png not found");
                }
                pauseOverlay.setVisible(false);
            } catch (Exception e) {
                LOGGER.log(Level.SEVERE, "Error loading pause overlay", e);
            }
        }
            ...
            timeLine = new Timeline(new KeyFrame(
                    Duration.millis(400),
                    ignoredEvent-> moveDown(new MoveEvent(EventType.DOWN, EventSource.THREAD))
            ));
    ...
    //        add ghost panel into gui
            if (brickPanel.getParent() instanceof javafx.scene.layout.Pane) {
                ((javafx.scene.layout.Pane) brickPanel.getParent()).getChildren().addFirst(ghostPanel);
            }
        }
    ...
    //  update hold panel
        ...
        private Paint getFillColor(int i) {
            return switch (i) {
                ...
                case 1 -> Color.web("bde0fe");
                case 2 -> Color.web("ffc8dd");
                case 3 -> Color.web("d9ed92");
                case 4 -> Color.web("fff3b0");
                case 5 -> Color.web("ffadad");
                case 6 -> Color.web("ffc6ff");
                case 7 -> Color.web("ffd6a5");
                default -> Color.WHITE;
            };
        }
    ...
    //            Update ghost piece
        ...
        public void pauseGame(ActionEvent ignoredActionEvent) {
            gamePanel.requestFocus();
